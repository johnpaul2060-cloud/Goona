...
export default function KycStep3Screen() {
  const insets = useSafeAreaInsets()
  const existing = useWalletStore((s) => s.kyc.step3)
  const setKycStep3 = useWalletStore((s) => s.setKycStep3)

  const [nin, setNin] = useState(existing.nin)
  const [verifying, setVerifying] = useState(false)
  const [verified, setVerified] = useState(false)

  const isValid = nin.length >= 10
  ...
  const handleSubmit = () => {
    setKycStep3({ nin, completed: true })
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)
    ...
    <View style={styles.container}>
      ...
      >
        <Animated.View entering={FadeInDown.duration(600).springify()} style={[styles.header, { paddingTop: insets.top + 16 }]}>
          ...
          <Text style={styles.subtitle}>Verify your National Identification Number.</Text>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(80).springify()} style={styles.formCard}>
          <View style={styles.fieldGroup}>
            ...
            <TextInput style={styles.input} value={nin} onChangeText={setNin} keyboardType="numeric" placeholder="Enter 11-digit NIN" placeholderTextColor="#CBD5E1" maxLength={11} editable={!verified} />
          </View>
          ...
          >
            <LinearGradient colors={verified ? ['#16A34A', '#15803D'] : ['#2E7D32', '#1B5E20']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.verifyGrad}>
              {verifying ? (
                ...
                <Text style={styles.verifyText}>Verify NIN</Text>
              )}
            </LinearGradient>
          </TouchableOpacity>
        </Animated.View>
