...
function DailyChallenge() {
  const [selected, setSelected] = useState<number | null>(null)
  const options = ['Use feed reserves', 'Reduce daily feeding', 'Borrow feed from partner', 'Delay operations']
  return (
    <Animated.View entering={FadeInUp.duration(500).delay(950).springify()}>
      <LinearGradient colors={['#FEF9C3', '#FFF7ED']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={dcStyles.card}>
        <View style={dcStyles.titleRow}>
          <Text style={dcStyles.title}>{'\u{1F9E0}'} What Would You Do?</Text>
        </View>
        <Text style={dcStyles.scenario}>Feed delivery delayed by 3 days. Your stock is running low. What's your move?</Text>
        {options.map((opt, i) => {
          const num = i + 1
          const isSelected = selected === num
          return (
            <TouchableOpacity key={num} activeOpacity={0.9} onPress={() => setSelected(num)} style={[dcStyles.opt, isSelected && dcStyles.optSelected]}>
              <View style={[dcStyles.radio, isSelected && dcStyles.radioSelected]}>
                {isSelected && <View style={dcStyles.radioInner} />}
              </View>
              <Text style={dcStyles.optText}>{opt}</Text>
            </TouchableOpacity>
          )
        })}
        {selected && (
          <Animated.View entering={FadeIn.duration(300)} style={dcStyles.reward}>
            <Text style={dcStyles.rewardText}>{'\u2728'} +150 XP &middot; Operational Rank +1</Text>
          </Animated.View>
        )}
      </LinearGradient>
    </Animated.View>
...
const dcStyles = StyleSheet.create({
  ...
  title: { fontWeight: '700', fontSize: 15, color: '#92400E' },
  scenario: { fontSize: 13, color: '#78350F', marginBottom: 12, lineHeight: 19 },
  opt: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, paddingHorizontal: 14, backgroundColor: 'rgba(255,255,255,0.5)', borderWidth: 1.5, borderColor: 'rgba(234,179,8,0.2)', borderRadius: 14, marginBottom: 6 },
  optSelected: { borderColor: '#D97706' },
  radio: { width: 18, height: 18, borderRadius: 9, borderWidth: 2, borderColor: '#D4A017', alignItems: 'center', justifyContent: 'center' },
  radioSelected: { borderColor: '#D97706', backgroundColor: '#D97706' },
  radioInner: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#fff' },
  optText: { fontSize: 13, fontWeight: '500', color: '#1B1B1B', flex: 1 },
  reward: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8 },
