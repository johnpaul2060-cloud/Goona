...
export default function BottomDock({ hidden }: { hidden?: boolean }) {
  ...
  const activeIndex = (() => {
    ...
    if (pathname.includes('records') || pathname.includes('sales') || pathname.includes('record-sale') || pathname.includes('create-batch') || pathname.includes('daily-records') || pathname.includes('batch-details')) return 2
